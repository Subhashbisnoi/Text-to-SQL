...
def get_sql_query(user_query):
    if "show tables" in user_query.lower():
...
def get_sql_query(user_query):
    ...
    if "show tables" in user_query:
        return "SELECT name FROM sqlite_master WHERE type='table';"
    
    if "show database" in user_query or "show databases" in user_query:
        return "PRAGMA database_list;"  # Returns the database file details

    # Use LLM for other SQL queries
